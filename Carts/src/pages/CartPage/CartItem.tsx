...
import { CartDetailsDto, useGetApiCartsApiMemberId } from '../../API';
...
const CartItem: React.FC<CartProps> = ({ initialCart }) => {
  ...
  const TestData = useGetApiCartsApiMemberId(2){
...
useEffect(() => {
  const fetchData = async () => {
    const memberId = 2; // 替換成你要獲取的會員ID
    const response = await useGetApiCartsApiMemberId(memberId);
